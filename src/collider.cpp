...
PropertyControlSystem *colliderProperty::getPcs()
{
    return p;
}
...
{
    std::vector<colliderProperty *>::const_iterator it = find(colliders.begin(), colliders.end(), this);
    colliders.erase(it);
}
...
void colliderProperty::setDraw(bool b)
{
    drawBody = b;
}
...
SDL_Rect colliderProperty::getBody()
{
    return body;
}
